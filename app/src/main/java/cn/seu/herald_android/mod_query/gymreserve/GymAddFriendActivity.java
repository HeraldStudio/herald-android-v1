...
public class GymAddFriendActivity extends BaseActivity implements SearchView.OnQueryTextListener {
    ...
    private void loadSearchResult(ArrayList<FriendModel> list) {
        listView_searchfriendrelust.setAdapter(new FriendModel.FriendAdapter(getBaseContext(), R.layout.mod_que_gymreserve__search_friend__item, list));
    ...
    public void addFriend(FriendModel friendModel) {
        try{
            CacheHelper.set("herald_gymreserve_recentfriendlist", getFriendJSONArray().put(friendModel.getJSONObject()).toString());
        }catch (JSONException e){
            e.printStackTrace();
        }
    }
