 ...
 *
 *  这里的 get-set 变量并非变量, 它本身不存储数据, 却包含了一个 get 函数和一个 set 函数, 对外显示成变量的形态.
 * 定义一个 get-set 变量之后, "取它的值", 即可自动调用预设的 get 函数; 给它"赋值", 即可自动调用预设的 set 函数.
 *
 *  当然 Java 不能重载运算符, 无法完全实现这种自动调用函数的功能, 我们还是要通过调用函数的方式来调用它的 get/set
 * 函数. 这样对于调用来说并不算很方便, 但定义的时候比较省事了. 所以这个类也许纯粹是为了简化 set/get 函数的成对定义
 * 而产生的吧.
 *
 * 定义方法:
 *
